...
class ControllerParties {
  ...
  static getList = async (req: Request, res: Response) => {
    ...
    if (type) {
      const info = data.record.parties.filter(p => p.roles.includes(type)).map(p => ({ id: p.id, name: p.name, roles: p.roles }));

      res.json({ data: info });
    } else {
      const info = data.record.parties.map(p => ({ id: p.id, name: p.name, roles: p.roles }));
      res.json({ data: info });
    }
  };
